...
class AckermannVision:
    ...
    def process_frame(self):
        ...
        # Apply region of interest to HSV mask
        roi_mask = self.region_selection(hsv_mask)

        # Apply Gaussian blur and Canny to mask before Hough
        blur = cv2.GaussianBlur(roi_mask, (5, 5), 0)
        edges = cv2.Canny(blur, 50, 150)

        # Hough line detection on mask
        lines = self.hough_transform(edges)

        angle = None
        road_center = None
        lane_img = cv2.cvtColor(roi_mask, cv2.COLOR_GRAY2BGR)  # Use HSV mask as base for drawing
        ...
        if lines is not None:
            ...
            if road_center is not None:
                # Calculate steering angle from lane center
            ...
            # Draw lane lines and center dot on the HSV mask image

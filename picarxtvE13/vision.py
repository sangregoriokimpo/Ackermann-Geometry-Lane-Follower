#vision.py

import cv2
import numpy as np

class AckermannVision:
    def __init__(self, camera_device='/dev/video0'):
        self.cap = cv2.VideoCapture(camera_device, cv2.CAP_V4L2)
        self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 320)
        self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 240)
        self.cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG'))
        self.cap.set(cv2.CAP_PROP_FPS, 30)

        if not self.cap.isOpened():
            raise IOError(f"Cannot open camera {camera_device}")

        self.minAngle = -30
        self.maxAngle = 30

    def release(self):
        if self.cap.isOpened():
            self.cap.release()

    def region_selection(self, image):
        mask = np.zeros_like(image)
        rows, cols = image.shape[:2]
        vertices = np.array([[
            [cols * 0.0, rows * 1.0],
            [cols * 0.0, rows * 0.5],
            [cols * 1.0, rows * 0.5],
            [cols * 1.0, rows * 1.0]
        ]], dtype=np.int32)

        cv2.fillPoly(mask, vertices, 255)
        return cv2.bitwise_and(image, mask)

    def hough_transform(self, image):
        return cv2.HoughLinesP(image, 1, np.pi / 180, 20, minLineLength=20, maxLineGap=500)

    def average_slope_intercept(self, lines):
        left_lines, right_lines, left_weights, right_weights = [], [], [], []
        for line in lines:
            for x1, y1, x2, y2 in line:
                if x1 == x2:
                    continue
                slope = (y2 - y1) / (x2 - x1)
                intercept = y1 - slope * x1
                length = np.hypot(x2 - x1, y2 - y1)
                if slope < 0:
                    left_lines.append((slope, intercept))
                    left_weights.append(length)
                else:
                    right_lines.append((slope, intercept))
                    right_weights.append(length)
        left_lane = np.dot(left_weights, left_lines) / np.sum(left_weights) if left_weights else None
        right_lane = np.dot(right_weights, right_lines) / np.sum(right_weights) if right_weights else None
        return left_lane, right_lane

    def pixel_points(self, y1, y2, line):
        if line is None:
            return None
        slope, intercept = line
        if abs(slope) < 0.01:
            return None
        x1 = int((y1 - intercept) / slope)
        x2 = int((y2 - intercept) / slope)
        return ((x1, int(y1)), (x2, int(y2)))

    def lane_lines(self, image, lines):
        left_lane, right_lane = self.average_slope_intercept(lines)
        y1 = image.shape[0]
        y2 = y1 * 0.6
        return self.pixel_points(y1, y2, left_lane), self.pixel_points(y1, y2, right_lane)

    def draw_lane_lines(self, image, lines, road_center=None):
        output = image.copy()
        for line in lines:
            if line is not None:
                cv2.line(output, *line, (255, 0, 0), 6)
        if road_center is not None:
            cv2.circle(output, (road_center, image.shape[0] - 20), 5, (0, 255, 0), -1)
        return output
    
    def process_full_frame(self):
        ret, frame = self.cap.read()
        if not ret or frame is None:
            return None, None

        frame = cv2.resize(frame, (320, 240))  # Keep resolution consistent

        # Use full frame instead of cropping
        hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        lower_hsv = np.array([0, 0, 226])
        upper_hsv = np.array([179, 255, 255])
        hsv_mask = cv2.inRange(hsv, lower_hsv, upper_hsv)

        roi_mask = self.region_selection(hsv_mask)  # Optional: still restrict to ROI
        blur = cv2.GaussianBlur(roi_mask, (5, 5), 0)
        edges = cv2.Canny(blur, 50, 150)
        lines = self.hough_transform(edges)

        lane_img = cv2.cvtColor(roi_mask, cv2.COLOR_GRAY2BGR)
        road_center = None
        angle = None

        if lines is not None:
            left_line, right_line = self.lane_lines(frame, lines)
            mid_x = frame.shape[1] // 2

            if left_line and right_line:
                road_center = (left_line[1][0] + right_line[1][0]) // 2
            elif left_line:
                road_center = left_line[1][0] + 100
            elif right_line:
                road_center = right_line[1][0] - 100

            if road_center is not None:
                error = road_center - mid_x
                steering_value = error / mid_x
                steering_value = np.clip(steering_value, -1.0, 1.0)
                angle = ((steering_value + 1) / 2) * (self.maxAngle - self.minAngle) + self.minAngle

            lane_img = self.draw_lane_lines(lane_img, [left_line, right_line], road_center)

        return angle, lane_img


    def process_frame(self):
        ret, frame = self.cap.read()
        if not ret or frame is None:
            return None, None, None

        frame = cv2.resize(frame, (320, 240))  # Ensure consistent resolution

        # --- HSV Mask creation using full frame ---
        hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        lower_hsv = np.array([0, 0, 226])
        upper_hsv = np.array([179, 255, 255])
        hsv_mask = cv2.inRange(hsv, lower_hsv, upper_hsv)

        # Apply region of interest (optional, can be removed entirely if undesired)
        # roi_mask = self.region_selection(hsv_mask)
        roi_mask = hsv_mask.copy()


        # Apply Gaussian blur and Canny edge detection
        blur = cv2.GaussianBlur(roi_mask, (5, 5), 0)
        edges = cv2.Canny(blur, 50, 150)

        # Detect lines using Hough transform
        lines = self.hough_transform(edges)

        angle = None
        road_center = None
        lane_img = cv2.cvtColor(roi_mask, cv2.COLOR_GRAY2BGR)  # For drawing lines on

        if lines is not None:
            left_line, right_line = self.lane_lines(frame, lines)
            mid_x = frame.shape[1] // 2

            if left_line and right_line:
                road_center = (left_line[1][0] + right_line[1][0]) // 2
            elif left_line:
                road_center = left_line[1][0] + 100
            elif right_line:
                road_center = right_line[1][0] - 100

            if road_center is not None:
                error = road_center - mid_x
                steering_value = error / mid_x
                steering_value = np.clip(steering_value, -1.0, 1.0)
                angle = ((steering_value + 1) / 2) * (self.maxAngle - self.minAngle) + self.minAngle

            # Draw lane lines and center
            lane_img = self.draw_lane_lines(lane_img, [left_line, right_line], road_center)

        return angle, lane_img, frame  # Return original frame instead of top_half

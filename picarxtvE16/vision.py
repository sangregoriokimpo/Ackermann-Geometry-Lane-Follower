...
class AckermannVision:
    def __init__(self, camera_device='/dev/video0'):
        ...
        self.pid = PIDController(Kp=2.75, Ki=0.75, Kd=0.3, integral_limit=50)
    ...
    def process_frame(self):
        ...
        if not ret or frame is None:
            return None, None, None  # angle, lane image, top_half (for sign detection)
        ...
        half = height // 2
        top_half = frame[:half, :]      # (120, 320, 3) → for traffic sign detection
        bottom_half = frame[half:, :]   # (120, 320, 3) → for lane keeping

        # Lane detection pipeline on bottom_half
        ...
        if lines is not None:
            ...
            steering_output = self.pid.update(error, dt)
            steering_value = np.clip(steering_output / mid_x, -1.0, 1.0)
        ...
        # Return angle for steering, bottom_half with lane overlay, top_half for sign detection
